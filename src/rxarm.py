...
#   @_ensure_initialized
    ...
    def get_ee_pose(self):
        ...
        self.parse_pox_param_file()
        pos_matrix = FK_pox(joint_angles, self.M_matrix, self.S_list)
        ee_x = pos_matrix[0][3]
        ee_y = pos_matrix[1][3]
        ee_z = pos_matrix[2][3]
        
        rotation_matrix = pos_matrix[:3, :3]
        rotation_object = R.from_matrix(rotation_matrix)
        phi, theta, psi = rotation_object.as_euler('ZYX', degrees=False)
        IK_joints = IK_geometric(np.array([ee_x, ee_y, ee_z, psi]))
        # print("psi:", psi)
        # print("IK_joints")
        # print(np.degrees(IK_joints))
        # print("real_joint_angles")
        # print(np.degrees(joint_angles))
        return [ee_x, ee_y, ee_z, phi, theta, psi]

    def compare_FKIK(self):
        """!
        @brief      Compare the FK and IK functions.
        """
        joint_angles = self.get_positions()
        print("GT_joint_angles")
        print(joint_angles)
        ee_pose = self.get_ee_pose()
        # print("ee_pose")
        # print(ee_pose)
        IK_angles = IK_geometric(ee_pose)
        print("IK_angles")
        print(IK_angles)
        dif_angles = np.degrees(joint_angles - IK_angles)
        print("dif_angles")
        print(dif_angles)
    ...
    def parse_pox_param_file(self):
        ...
        #TODO Done
        try:
            with open(self.pox_config_file, 'r') as file:
                ...
                for row in reader:
                    if not row or row[0].startswith('#'):
                        if row[0].startswith('# M matrix'):
                            m_matrix_read = True
                        elif row[0].startswith('# screw vectors'):
                            m_matrix_read = False
                        continue

                    if m_matrix_read:
                        m_matrix.append([float(x) for x in row])
                    else:
                        s_list.append([float(x) for x in row])

                self.M_matrix = np.array(m_matrix)
                self.S_list = np.array(s_list)
                return 0
        except Exception as e:
            print(f"Error reading POX config file: {e}")
            return -1
